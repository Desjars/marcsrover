...
def realsense_draw(sample: D435I) -> None:
    ...
    depth = XY8_to_BGR8(np.frombuffer(bytes(sample.depth), dtype=np.uint8).reshape((240, 320, 2)))

    rgb, depth = cv2.resize(rgb, (480, 360)), cv2.resize(depth, (480, 360))
    rgb, depth = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR), cv2.cvtColor(depth, cv2.COLOR_RGB2BGR)
