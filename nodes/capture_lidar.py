...
#Import LidarFrame from message.py
from message import LidarFrame

class CaptureLidar:
    # Start a scan
    def __init__(self, port='/dev/ttyUSB0', bauderate=256000, timeout=3, pwm=500, timesleep=2, topic='marcsrover/lidar'):
    ...
    def run(self):
        ...
        for count, scan in enumerate(scan_generator()):
            print(scan)
            scan = LidarFrame(start_flag=scan['start_flag'], angle=scan['angle'], quality=scan['quality'], distance=scan['distance'])
            #self.camera_publisher.put(LidarFrame.serialize(scan))
...
if __name__ == "__main__":
        capture_lidar=CaptureLidar()
        capture_lidar.run()

        # Stop the motor and disconnect the LiDAR
        capture_lidar.stop()
        capture_lidar.set_motor_pwm(0)
        capture_lidar.disconnect()
